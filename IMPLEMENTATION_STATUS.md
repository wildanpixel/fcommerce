...
- 2026-07-13: Fixed Keyword Projects and Project Inspector delete actions with a simpler confirmation flow and pending-state protection, restricted Activity to guided collection screens only, and restyled circular icon controls, Vault Metrics, Report History, and dark-mode surfaces for parity with the soft light-mode UI.
...
- 2026-07-27: Replaced Part 3 single-store AI ranking with a multi-store Key Store Page List. Every approved store now has its own Store Home Page, Store Data, 1-star/5-star ratings, categories, best sellers, TikTok evidence, and optional popular-product/banner actions, with shop-ID discovery reused across subsequent targets.
